...
- 每个端点统计里的 `saved`（套餐端点）是「走预付端点的流量」按当时忙/闲单价**本应花掉的
  现金**——估算值，不是账单，按端点自身币种记账，不做跨币种求和。
- `cold_starts` 统计成功请求中 prompt 未命中上游缓存的次数；当会话在端点之间跳动时，
  这是最该盯的一个数。
- 端点行上的「处理中」标记表示该端点**此刻有请求正在上游执行**（含整个流式过程），
  不是完成量的统计。计数带了时间戳并会老化：一个连接被异常中断遗留的标记会在一小时后
  归零，而不是永远把空闲端点显示成忙碌。
- 这些计数是**累计值**，不是「本次运行」的值：它们会写进状态文件，重启服务不会清零。
  ...
  于是被优先消耗。校准进行中时重置会被拒绝（提示先取消校准）；重置同时会丢弃未完成的
  校准起点读数，因为它们引用的计数器刚被清零。
