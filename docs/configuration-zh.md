...
| `quota.used_percent` + `quota.used_at` | 用厂商后台的已用百分比校准本地账本。见下。 |
...
配上 `cycle_day` 之后，「本周期已用」= 从本周期起点累计到今天。周期边界由锚点日算出，因此
**本地也能知道重置时刻**——滑动窗口从来给不出这个值，所以控制台现在能显示与厂商一致的重置
倒计时和投影。

### 校准：让本地数字与后台对齐

路由器只能看到**经过它自己转发**的流量。如果这个 key 还被别的工具用，或者在配置这个端点之前
就已经花掉了一部分，本地百分比必然偏低。用一次校准补上：

```yaml
quota: { ..., cycle_day: 26, used_percent: 43.5, used_at: 1767225600 }
```

含义是「`used_at` 这一刻，厂商后台显示 43.5%」。当前周期已用量 =
`43.5% × 上限 + 从 used_at 到现在路由器转发的量`。

填**百分比**而不是金额，因为后台显示的就是百分比，你照抄即可，换算交给路由器。

`used_at` 不是多余的记账字段，它是**校准的边界**：没有它，这个百分比会被加进每一个周期，
凭空造出厂商从未记录的用量。有了它，进入下一周期后校准自动失效——这正是要的行为，所以到期
不需要手动清零。反过来，只填百分比不填时刻会被拒绝并给出警告，因为它永远无法失效。
