...
### 5 小时与周窗口由「使用」开启，不由时钟决定

月窗口跟着日历走，另外两个不跟。一个窗口在周期走完时结束，而**下一个窗口由结束之后到达的第一条**
**请求开启**——因此两段窗口之间的空闲时间不属于任何窗口，窗口跨度事先也不可知。

把从后台抄来的倒计时按固定网格往前滚，**表达不了**这件事：你一停一用，网格立刻偏。厂商自己在拒绝
请求时会把这个规则说出来（「it will reset at …」），而只有按使用锚定，那个时刻才复现得出来。

由此有两点：

- 窗口旁的金额 = 账本自该窗口开启以来的累加，所以与后台百分比天然对齐——前提是窗口内的流量确实
  都经过路由器。
- 窗口结束到下一次请求之间没有周期在跑：此时窗口读作 0 且不显示重置时刻，而不是给一个已经过去的
  边界倒计时。

`CD 5h` / `CD week`（后台的重置倒计时）仍然即填即生效，但现在它修正的是窗口的**起点**（倒计时减
一个周期），而不是喂给网格。只填一个不会动另一个；填 0 表示清除。
...
一次读数给每个窗口一个**基准**，此后该窗口读作「基准 + 账本自那以后记下的钱」。基准以**金额**
存储——在记录读数的那一刻算好（`总额 × 百分比 / 100`）——而不是存那个百分比。原因有两点：额度
的计量主体本来就是钱，读的时候不需要再换算；而存百分比则会在每次读取时重新乘以套餐总额，于是
日后修正套餐值会**悄悄改变一个描述既成事实的基准**。

同一次读数还有第二个互不相干的用途：**「同步剩余额度」**把基准重新对齐到后台当前显示的值。它
只动基准，不动其它任何东西——校准推出来的单价与窗口总额是**套餐的属性**，不是当前水平的属性，
所以同步永远不会让你丢掉一次校准。当额度因为路由器看不见的原因发生变化（比如同一个 key 在别处
也在用）时，用它。

第三个动作只需**一个读数**就能定出窗口额度：通常要两次读数是因为第一次读数时的金额未知，而现在窗口
是从开启它的那条请求起被跟踪的，金额已知，于是 `额度 = 窗口内已转发的金额 ÷ 读数` 直接确定。窗口已
关闭、或读数为 0 时会被拒绝而不是猜一个值。它只在窗口内全部流量都经过路由器时才准确——没经过的部分
会让额度偏小，这是安全的一侧（窗口会显得比实际用得多）。

唯一会**让一次校准退场**的改动是修改 `quota.monthly`：两个窗口总额都与它成正比，改完之后它们
会差同一个比例，而基准金额停在原处，两者只能靠改写历史才能调和。此时校准是被**搁置**而不是删除：
账本（不需要总额）接管显示，界面会说明原因；把月费改回去，校准即恢复。修改单价或促销倍率则
**不会**影响窗口总额（它们在推算中约掉了），只改变此后请求的计价。
...
`peak_multiplier` 作用于忙时窗口；`promo_multiplier` 是同时作用于三项单价的**整体倍率**，用于促销或
套餐整体调整。两者分开是因为它们回答的问题不同：忙闲是**时钟**的属性，促销是**套餐**的属性、
在任何时刻都生效。缺省或不可用（0、负数）都按 1.0 处理。

修改任何单价都只影响**之后的**请求。每条账本样本记录的是该请求在转发当时按**当时单价**算出的钱，
这个数字是冻结的：修正单价或新增促销都无法改写一个已经付过费的周期。这也是可以随时加促销的原因。
