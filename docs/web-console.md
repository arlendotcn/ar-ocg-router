...
| **Endpoints** | Add, edit and remove endpoints; **drag rows to set the consumption order**; **enable/disable with immediate effect**; **duplicate an endpoint**; copy a ready-made `curl`; run a **connectivity self-test** (key, `/models`, quota or balance, one minimal request) that returns evidence-based suggestions; a **quota calibration wizard** derives a no-usage-API plan's real allowance from two console readings. |
