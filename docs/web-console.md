...
| Page | What it does |
| --- | --- |
| **Overview** | Peak/off-peak state with a countdown to the next change, request/error/stream/retry counters, per-endpoint health and cooldown with a live **"in flight" state**, quota meters for all three windows (with a reset countdown and a projection when the ledger has observed the cycle from its start), per-currency spend, tokens, cache hits, latency, last error. Refreshes every 2 s and can be paused. |
| **Endpoints** | Add, edit and remove endpoints; **drag rows to set the consumption order**; **enable/disable with immediate effect**; **duplicate an endpoint**; copy a ready-made `curl`; run a **connectivity self-test** (key, `/models`, quota or balance, one minimal request) that returns evidence-based suggestions; a **quota calibration wizard** derives a no-usage-API plan's real allowance from two console readings, or from one reading once a window is being tracked, and **syncs the level** to what the console shows now. |
| **Policy** | Routing mode, off-peak preference, surplus threshold and projection, exhaustion threshold, session affinity, every cooldown and skip value, and the compatibility switches. Includes a **candidate order preview** that simulates a request against the current clock and quota, asked **per protocol** (chat / responses / anthropic) because an endpoint only takes part when its `modes` covers the protocol the client used - the preview says which config it answered for, and warns when the page holds unsaved changes. |
